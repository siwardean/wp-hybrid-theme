Feature: WordPress code scaffolding
  ...
  Scenario: Scaffold hybrid_theme code for a theme
    Given a WP install
    And I run `wp theme path`
    And save STDOUT as {THEME_DIR}

    # Allow for warnings to be generated due to https://github.com/wp-cli/scaffold-command/issues/181
    When I try `wp scaffold _s hybrid_theme-theme`
    Then STDOUT should contain:
      """
      Success: Created theme 'Starter-theme'.
      """
    And the {THEME_DIR}/hybrid_theme-theme/style.css file should exist
    And the {THEME_DIR}/hybrid_theme-theme/.editorconfig file should exist

  @require-php-5.6 @require-wp-4.6
  Scenario: Scaffold hybrid_theme code for a theme with sass
    Given a WP install
    And I run `wp theme path`
    And save STDOUT as {THEME_DIR}

    # Allow for warnings to be generated due to https://github.com/wp-cli/scaffold-command/issues/181
    When I try `wp scaffold _s hybrid_theme-theme --sassify`
    Then STDOUT should contain:
      """
      Success: Created theme 'Starter-theme'.
      """
    And the {THEME_DIR}/hybrid_theme-theme/sass directory should exist

  @require-php-5.6 @require-wp-4.6
  Scenario: Scaffold hybrid_theme code for a WooCommerce theme
    Given a WP install
    And I run `wp theme path`
    And save STDOUT as {THEME_DIR}

    # Allow for warnings to be generated due to https://github.com/wp-cli/scaffold-command/issues/181
    When I try `wp scaffold _s hybrid_theme-theme --woocommerce`
    Then STDOUT should contain:
      """
      Success: Created theme 'Starter-theme'.
      """
    And the {THEME_DIR}/hybrid_theme-theme/woocommerce.css file should exist
    And the {THEME_DIR}/hybrid_theme-theme/inc/woocommerce.php file should exist

  @require-php-5.6 @require-wp-4.6 @require-mysql
  Scenario: Scaffold hybrid_theme code for a theme and activate it
    Given a WP install
    # Allow for warnings to be generated due to https://github.com/wp-cli/scaffold-command/issues/181
    When I try `wp scaffold _s hybrid_theme-theme --activate`
    Then STDOUT should contain:
      """
      Success: Switched to 'Starter-theme' theme.
      """

  Scenario: Scaffold hybrid_theme code for a theme with invalid slug
    Given a WP install
  ...
  @require-php-5.6 @require-wp-4.6
  Scenario: Scaffold hybrid_theme code for a theme and network enable it
    Given a WP multisite install
    # Allow for warnings to be generated due to https://github.com/wp-cli/scaffold-command/issues/181
    When I try `wp scaffold _s hybrid_theme-theme --enable-network`
    Then STDOUT should contain:
      """
  ...
  @require-php-5.6 @require-wp-4.6 @require-mysql
  Scenario: Scaffold hybrid_theme code for a theme, but can't unzip theme files
    Given a WP install
