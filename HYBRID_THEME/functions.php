<?php
/**
 * HYBRID_THEME functions and definitions
 *
 * @link https://developer.wordpress.org/themes/basics/theme-functions/
 *
 * @package HYBRID_THEME
 */

if ( ! defined( 'HYBRID_THEME_VERSION' ) ) {
	// Replace the version number of the theme on each release.
	define( 'HYBRID_THEME_VERSION', '0.1.0' );
}
...
function hybrid_theme_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary Menu', 'hybrid_theme' ),
			'menu-2' => esc_html__( 'Footer Menu', 'hybrid_theme' ),
		)
	);
 ...
 */
function hybrid_theme_image_size() {

	add_image_size( 'fullwidth', 1200 ); // 1200 pixels wide with unlimited height.
	add_image_size( 'team', 370, 370, true ); // true = cropped.
}
add_action( 'after_setup_theme', 'hybrid_theme_image_size' );

/**
 * Enqueue scripts and styles.
 */
function hybrid_theme_scripts() {
	// styles.
	wp_enqueue_style( 'hybrid_theme-wp', get_stylesheet_uri(), array(), HYBRID_THEME_VERSION );
	wp_enqueue_style( 'modern-normalize', '//cdn.jsdelivr.net/npm/modern-normalize@3.0.1/modern-normalize.min.css', array(), HYBRID_THEME_VERSION );
	// wp_enqueue_style( 'hybrid_theme-style', get_template_directory_uri() . '/assets/css/style.css', array(), HYBRID_THEME_VERSION );
	// javascript.
	// wp_enqueue_script( 'hybrid_theme-script', get_template_directory_uri() . '/assets/js/script.min.js', array( 'jquery' ), HYBRID_THEME_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'hybrid_theme_scripts' );

/**
 * Enqueue scripts and styles for admin UI.
 */
function hybrid_theme_admin_scripts() {
	wp_enqueue_style( 'hybrid_theme-admin-style', get_template_directory_uri() . '/assets/css/admin.css', array(), HYBRID_THEME_VERSION );
}
add_action( 'admin_enqueue_scripts', 'hybrid_theme_admin_scripts' );
 ...
 */
function hybrid_theme_search_form( $form ) {
...
add_filter( 'get_search_form', 'hybrid_theme_search_form', 40 );
 ...
 */
function hybrid_theme_custom_html_title( $title_parts_array ) {
	if ( is_home() ) {
...
add_filter( 'document_title_parts', 'hybrid_theme_custom_html_title', 10 );
